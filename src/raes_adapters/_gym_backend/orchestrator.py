"""Neutral portable orchestration state for a gym-backend episode."""
...
class GymEpisodeOrchestrator(object):
    ...
    def start(
        ...
        if unsupported:
            ...
            return ApplyResult(
                ...
                diagnostics=[
                    Diagnostic(
                        code=f"{self._name}.orchestration.unsupported-resource",
                        domain="orchestration",
                        address=diagnostic_address(operation.address, fallback=f"/{self._name}"),
                        message=(
                            "The selected backend orchestrator does not support this "
    ...
    def _workflow_result(
        ...
        return WorkflowExecutionStateModel(
            ...
            run_id=f"{payload.get('name', f'{self._name}-episode')}-run",
            started_at=now,
            updated_at=now,
...
__all__ = ["GymEpisodeOrchestrator"]
